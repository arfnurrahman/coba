...
// diagnostic/hack tambahan. Cuma logic inti: trigger audio & kuis saat marker
// terdeteksi, seperti pola di file referensi yang terbukti berfungsi.
function initArMarkerListeners() {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        targetEl.addEventListener('targetFound', () => {
            ...
            if (isMarkerLengkapTerjawab(i)) return;

            if (!markerTriggered[i]) {
                // Marker ini BARU PERTAMA KALI terdeteksi — mainkan audio dari awal.
                playAudioForTarget(i);
            } else {
                // Marker ini SUDAH PERNAH terdeteksi sebelumnya (mungkin sempat
                // hilang dari kamera lalu di-scan ulang) — lanjutkan audio dari
                // titik terakhir sebelum berhenti, JANGAN diulang dari awal.
                const a = document.getElementById('assess-audio-' + i);
                if (a && a.paused && !a.ended) {
                    a.play().catch(err => console.error(`[AR] Gagal melanjutkan audio marker ${i + 1}:`, err));
                }
        ...
        targetEl.addEventListener('targetLost', () => {
            ...
            // Audio "menyatu" dengan gambar 3D: begitu marker hilang dari kamera,
            // audio-nya ikut berhenti (di-pause, BUKAN direset ke awal) — supaya
            // tidak bertabrakan dengan audio marker lain yang di-scan berikutnya.
            // Nanti saat marker ini di-scan ulang, audio lanjut dari titik ini.
            if (!isMarkerLengkapTerjawab(i)) {
                const a = document.getElementById('assess-audio-' + i);
                if (a) a.pause();
            }
        });
...
// ================= EDITOR VIDEO (Admin) — DIROMBAK: dikelompokkan per
// dimensi (1-8), tiap dimensi bisa punya banyak video. Ada tombol "+ Tambah
// Video" per dimensi, dan tombol Hapus per video. =================
...
function renderEditorVideoList() {
    ...
    for (let dimensiNo = 1; dimensiNo <= 8; dimensiNo++) {
        const videos = editorVideoCache.filter(v => v.dimensiNo === dimensiNo);
        html += `
            <div style="margin-top:16px;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
                    <h3 style="color:#C9A961; font-size:14px; margin:0;">${dimensiNo}. ${NAMA_8_DIMENSI[dimensiNo - 1]}</h3>
                    <button type="button" style="width:auto; padding:5px 10px; font-size:11px; margin:0;" onclick="bukaTambahVideo(${dimensiNo})">+ Tambah Video</button>
                </div>
                ${videos.length === 0 ? '<p style="font-size:12px; color:#cbd5e1; margin:4px 0;">Belum ada video.</p>' : ''}
                ${videos.map(v => `
                    <div style="display:flex; justify-content:space-between; align-items:center; gap:8px; padding:8px 0; border-bottom:1px solid rgba(201,169,97,0.15);">
                        <div style="flex:1; font-size:13px;">
                            <strong>${v.judul}</strong><br>
                            <span style="color:#cbd5e1;">youtu.be/${v.youtubeId}</span>
                        </div>
                        <button type="button" style="width:auto; padding:6px 10px; font-size:12px; margin:0;" onclick="bukaEditVideo(${v.id})">Edit</button>
                        <button type="button" style="width:auto; padding:6px 10px; font-size:12px; margin:0; background:#450a0a; color:#fca5a5;" onclick="hapusVideo(${v.id})">Hapus</button>
                    </div>
                `).join('')}
            </div>
        `;
...
function bukaTambahVideo(dimensiNo) {
    document.getElementById('editVideoId').value = '';
    document.getElementById('editVideoDimensiNo').value = dimensiNo;
    document.getElementById('editVideoTitle').textContent = `Tambah Video — Dimensi ${dimensiNo}. ${NAMA_8_DIMENSI[dimensiNo - 1]}`;
    document.getElementById('editVideoJudul').value = '';
    document.getElementById('editVideoLink').value = '';
    document.getElementById('editVideoDeskripsi').value = '';
    document.getElementById('btnSimpanVideo').textContent = 'Tambah Video';

    document.getElementById('editVideoModal').style.display = 'flex';
}

function bukaEditVideo(id) {
    const video = editorVideoCache.find(v => v.id === id);
    if (!video) {
    ...
    document.getElementById('editVideoDimensiNo').value = video.dimensiNo;
    document.getElementById('editVideoTitle').textContent = `Edit Video — Dimensi ${video.dimensiNo}. ${NAMA_8_DIMENSI[video.dimensiNo - 1]}`;
    ...
    document.getElementById('btnSimpanVideo').textContent = 'Simpan Perubahan';
...
function hapusVideo(id) {
    ...
    const params = new URLSearchParams();
    params.append('action', 'deleteVideo');
    params.append('password', sessionStorage.getItem('admin_password') || '');
    params.append('id', id);
...
function simpanEditVideo() {
    const id = document.getElementById('editVideoId').value; // kosong = mode tambah
    const dimensiNo = document.getElementById('editVideoDimensiNo').value;
    ...
    const params = new URLSearchParams();
    params.append('action', id ? 'updateVideo' : 'addVideo');
    params.append('password', sessionStorage.getItem('admin_password') || '');
    if (id) params.append('id', id);
    params.append('dimensiNo', dimensiNo);
    params.append('judul', judul);
    ...
    fetch(urlScript, { method: 'POST', body: params })
        ...
        .finally(() => {
            btn.disabled = false;
            btn.textContent = id ? 'Simpan Perubahan' : 'Tambah Video';
        });
}
...
// data di-fetch dari Google Sheets (tab "Video"). Setiap dimensi bisa punya
// LEBIH DARI 1 video (dikelompokkan lewat kolom DimensiNo), jadi ada 2 lapis
// navigasi: tab dimensi (1-8, selalu tetap) + sub-tab video (kalau dimensi
// itu punya lebih dari 1 video). =================

// Nama 8 dimensi TETAP (independen dari jumlah video per dimensi) — dipakai
// untuk label tab dimensi supaya selalu ada 8 tab walau video-nya 0/1/lebih.
const NAMA_8_DIMENSI = [
...
let videoDimensiAktif = 1; // 1-8
let videoSubIndexAktif = 0; // index video ke berapa DALAM dimensi ini (kalau lebih dari 1)
let VIDEO_LIST = []; // semua video mentah dari server: {id, dimensiNo, judul, youtubeId, deskripsi}
...
function getVideoUntukDimensi(dimensiNo) {
    return VIDEO_LIST.filter(v => v.dimensiNo === dimensiNo);
}

function renderVideoTabs() {
    const tabsEl = document.getElementById('videoTabs');
    if (!tabsEl) return;
    tabsEl.innerHTML = NAMA_8_DIMENSI.map((nama, i) => `
        <button type="button" class="lms-tab ${(i + 1) === videoDimensiAktif ? 'lms-tab-active' : ''}" onclick="pilihVideoDimensi(${i + 1})">
            ${i + 1}. ${nama}
        </button>
    `).join('');
}
...
function renderVideoContent() {
    ...
    const videos = getVideoUntukDimensi(videoDimensiAktif);
    const namaDimensi = NAMA_8_DIMENSI[videoDimensiAktif - 1];

    if (videos.length === 0) {
        contentEl.innerHTML = `
            <h3 class="lms-dimensi-title">${videoDimensiAktif}. ${namaDimensi}</h3>
            <p style="color:#C9A961; font-size:13px;">Belum ada video untuk dimensi ini.</p>
        `;
        contentEl.scrollTop = 0;
        return;
    }

    if (videoSubIndexAktif >= videos.length) videoSubIndexAktif = 0;
    const d = videos[videoSubIndexAktif];

    // Sub-navigasi video HANYA muncul kalau dimensi ini punya lebih dari 1 video
    const subTabsHtml = videos.length > 1
        ? `<div class="video-sub-tabs">${videos.map((v, i) => `
                <button type="button" class="video-sub-tab ${i === videoSubIndexAktif ? 'video-sub-tab-active' : ''}" onclick="pilihVideoSub(${i})">
                    Video ${i + 1}
                </button>
            `).join('')}</div>`
        : '';
    ...
    const embedHtml = embedSrc
        ...
        : `<div style="position:absolute; top:0; left:0; width:100%; height:100%; display:flex; align-items:center; justify-content:center; background:rgba(0,0,0,0.2); color:#C9A961; font-size:13px; text-align:center; padding:16px; box-sizing:border-box;">Video belum tersedia (menyusul)</div>`;

    contentEl.innerHTML = `
        <h3 class="lms-dimensi-title">${videoDimensiAktif}. ${namaDimensi}</h3>
        ${subTabsHtml}
        <p style="font-size:14px; font-weight:700; color:#F8FAFC; margin:0 0 8px 0;">${d.judul}</p>
...
function pilihVideoDimensi(dimensiNo) {
    videoDimensiAktif = dimensiNo;
    videoSubIndexAktif = 0;
    renderVideoTabs();
    renderVideoContent();
}

function pilihVideoSub(index) {
    videoSubIndexAktif = index;
    renderVideoContent();
}
